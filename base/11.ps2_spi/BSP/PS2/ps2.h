...
#define PS_PIN_DI 	GPIO_Pin_14 //MISO
#define PS_PIN_DO 	GPIO_Pin_15 //MOSI
...
void PS2_Cmd(u8 CMD);		  // Sending commands to the controller
u8 PS2_DataKey(void);		  // Read the key value
u8 PS2_AnologData(u8 button); // Get the analog value of a joystick
void PS2_ClearData(void);	  // Clear the data buffer
void PS2_Vibration(u8 motor1, u8 motor2);// Vibration setting motor1 0xFF on, others off, motor2 0x40~0xFF
